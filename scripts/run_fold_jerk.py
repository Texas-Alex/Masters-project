    ...
    def __init__(self, args):
        ...
        if os.path.isfile(presto_db):
            ...
            for i in range(len(file_cand)):
                ...
                cmd = 'prepfold -psrfits -noscales -nooffsets -noweights -noxwin -ncpus 1 -mask rfi_root_rfifind.mask -dm {0} -o {1} -nsub {2} -npart {3} {4} -n {5} -npfact {6} -ndmfact {7} -rzwcand {8} -rzwfile {9} -p {10} {11}'.format(DM_f[i], fold_out, nsub, npart, otheropts, N, Mp, Mdm, num_cand[i], file_cand[i], p_f[i], self.infile)
                print (cmd)
