...
class obs_info_class:
    ...
    def __init__(self, args):
        ...
        npart = 24  #int(length/self.tpart)        # The number of sub-integrations to use for the period search
        ...
        if os.path.isfile(presto_db):
            ...
            ######### sift ###########
            proc = subprocess.Popen(['python3', '/home/psr/software/presto/bin/ACCEL_sift.py'], shell=False, stdout=subprocess.PIPE)
            ...
            for i in range(len(file_cand)):
                fold_out = self.infile[0] + '_DM' + str(DM_f[i]) + '_P' + str(p_f[i])
                ...
                cmd = 'prepfold -psrfits -nodmsearch -noscales -nooffsets -noweights -noxwin -ncpus 1 -mask rfi_root_rfifind.mask -dm {0} -o {1} -nsub {2} -npart {3} {4} -n {5} -npfact {6} -ndmfact {7} -accelcand {8} -accelfile {9} {10}'.format(DM_f[i], fold_out, nsub, npart, otheropts, N, Mp, Mdm, num_cand[i], file_cand[i]," ".join(self.infile))
                print (cmd)
...
# required
parser.add_argument('input_filename',      metavar='filename', nargs="+" , help='Input file name')
# not required
...
parser.add_argument('-tpart',    '--time_part',      metavar='30.',            help='Sub-integration time for period search (npart = length/tpart) in seconds', default = 60., type = float)
