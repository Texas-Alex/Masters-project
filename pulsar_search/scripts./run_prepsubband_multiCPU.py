...
class obs_info_class:
    ...
    def __init__(self, args):
        ...
        print ('Input file name list: %s'% " ".join(self.infile))
        ...
        if os.path.isfile(presto_db):
            ...
            for i in range(len(dmlos)):
                ...
                for j in range(call[i]):
                    ...
                    cmd_dedispersion = 'prepsubband -psrfits -noscales -nooffsets -noweights -ncpus 1 -mask rfi_root_rfifind.mask -lodm {0} -dmstep {1} -numdms {2} -downsamp {3} -numout {4} -nsub {5} -o {6} {7}'.format(lodm_temp, dmstep, dm_call[i], downsamp[i], self.nout, self.nsubDM, self.infile[0], " ".join(self.infile))
